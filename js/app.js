...
      const lbl = Math.round((i/steps)*maxVal);
      ctx.fillText(lbl, padding.left-20, y+4);
    }

    // barras
    const barWidth = gw / (values.length * 1.5);
    const barSpacing = barWidth / 2;
    values.forEach((val, i)=>{
      const x = padding.left + i*(barWidth+barSpacing) + barSpacing/2;
      const barHeight = (val / maxVal) * gh;
      ctx.fillStyle = opts.color || '#c29d67';
      ctx.fillRect(x, padding.top + gh - barHeight, barWidth, barHeight);
    });
    // labels eixo X
    ctx.fillStyle = 'rgba(0,0,0,0.5)';
...
      const y = padding.top + gh + 18;
      ctx.fillText(lbl, x, y);
    });
  }
  // O fechamento da função drawBarChart() e do bloco principal (IIFE)
  })();